# How-To
This document describes how to develop for `MipVmsApi`.
...
There are multiple scripts created for performing different actions for developing `MipVmsApi`.
- `build-module.ps1`
  - Builds the module DLL (`./bin/MipVmsApi.private.dll`), creates the exported cmdlets and documentation, generates custom cmdlet test stubs and exported cmdlet example stubs, and updates `./MipVmsApi.psd1` with Azure profile information.
  - **Parameters**: [`Switch` parameters]
    - `-Run`: After building, creates an isolated PowerShell session and loads `MipVmsApi`.
...
- `run-module.ps1`
  - Creates an isolated PowerShell session and loads `MipVmsApi` into the session.
